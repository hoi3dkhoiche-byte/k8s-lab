...
const App = {
    ...
    updateAuthUI() {
        ...
        if (token) {
            const user = Api.auth.getUser() || { username: 'Tài khoản' };
            ...
            userMenu.innerHTML = `
                <button class="text-white hover:text-gold transition flex items-center gap-2">
                    <i class="fa-regular ${isAdmin ? 'fa-id-badge text-gold' : 'fa-user'}"></i>
                    <span class="text-sm hidden md:inline font-medium">${displayName} ${isAdmin ? '<span class="text-xs bg-gold/20 text-gold px-2 py-0.5 rounded-full border border-gold/40">Admin</span>' : ''}</span>
                </button>
                <div class="absolute right-0 mt-2 w-52 glass rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                    ${isAdmin ? `
                        <a href="admin.html" class="block px-4 py-3 text-sm text-gold font-semibold hover:bg-gold/20 border-b border-gold/20 flex items-center gap-2">
                            <i class="fa-solid fa-gauge"></i> Quản Trị Hệ Thống
                        </a>
                    ` : ''}
                    <a href="products.html" class="block px-4 py-3 text-sm text-white hover:bg-gold/20 hover:text-gold border-b border-gold/10">Tất cả sản phẩm</a>
            ...
            // Add Admin button to desktop nav if present
            const navLinks = document.querySelector('nav .hidden.md\\:flex');
            if (navLinks && isAdmin && !document.getElementById('adminNavLink')) {
                const adminLink = document.createElement('a');
                adminLink.id = 'adminNavLink';
                adminLink.href = 'admin.html';
                adminLink.className = 'text-gold hover:text-gold-hover font-semibold transition flex items-center gap-1.5';
                adminLink.innerHTML = '<i class="fa-solid fa-shield-halved text-xs"></i> Quản trị';
...
function renderProducts(products, container) {
    ...
    container.innerHTML = products.map(p => {
        ...
        const imgUrl = p.img || (p.images && p.images.length > 0 ? p.images[0] : 'images/cat-cues.jpg');
        ...
        <div class="glass product-card flex flex-col h-full bg-navy/40 rounded-2xl overflow-hidden hover:border-gold/60 transition-all duration-300 group">
            <a href="product.html?id=${p.id}" class="product-img-wrapper block h-48 sm:h-56 relative bg-black/50 overflow-hidden cursor-pointer">
                <img src="${imgUrl}" alt="${p.name}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onerror="this.src='images/cat-cues.jpg'">
