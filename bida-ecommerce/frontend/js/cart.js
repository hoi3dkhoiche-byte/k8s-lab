...
async function loadCart() {
    ...
    try {
        ...
        const itemsHtml = cart.items.map(item => {
            ...
            subtotal += itemTotal;
            return `
                <div class="glass p-4 rounded-xl flex gap-4 items-center mb-4 transition-all hover:border-gold/50" id="cart-item-${item.id}">
                    <img src="${item.img || 'images/cat-cues.jpg'}" class="w-24 h-24 object-cover rounded-lg bg-black/50">
                    <div class="flex-grow">
                        <a href="product.html?id=${item.id}" class="font-medium text-white hover:text-gold line-clamp-1">${item.name}</a>
                        <div class="text-gold font-semibold mt-1">${App.formatPrice(item.price)}</div>
                    </div>
                    <div class="flex items-center border border-gray-700 rounded bg-dark/50 mx-2">
                        <button onclick="updateCartQty(${item.id}, ${item.quantity - 1})" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white"><i class="fa-solid fa-minus text-xs"></i></button>
                        <span class="w-10 text-center text-sm font-medium">${item.quantity}</span>
                        <button onclick="updateCartQty(${item.id}, ${item.quantity + 1})" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white"><i class="fa-solid fa-plus text-xs"></i></button>
                    </div>
                    <div class="text-right min-w-[100px] hidden sm:block font-bold">
                        ${App.formatPrice(itemTotal)}
                    </div>
                    <button onclick="removeCartItem(${item.id})" class="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-red-500 transition rounded-full hover:bg-red-500/10">
