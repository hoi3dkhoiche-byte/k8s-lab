...
async def connect_to_kafka():
    ...
    try:
        ...
        logger.warning(fOrder Service: Lỗi kết nối Kafka: {e})
        producer = None
...
async def send_order_event(order_id, status, user_id):
    ...
    try:
        ...
        logger.info(fOrder Service: Đã gửi event cho order {order_id} trạng thái {status})
    except Exception as e:
        logger.error(fOrder Service: Lỗi khi gửi sự kiện Kafka: {e})
